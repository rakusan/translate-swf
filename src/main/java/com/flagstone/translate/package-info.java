 ...
 * <body>
 * <p>
 * The ASParser framework is a collection of classes used to implement a
 * compiler for Macromedia's ActionScript Version 1.0. ActionScript statements
 * are parsed to generate the byte-codes and actions that will be executed by
 * the Flash Player.
 * </p>
 *
 * <p>
 ...
 * data in the Flash (SWF) File Format which specifies the byte-codes and
 ...
 *
 * <p>
 * The version of Flash that the ASNodes will be encoded to is passed to the
 * root node. The file format of the data structures used to represent movie
 * clip events and button events changed when Macromedia released Version 6 an 7
 * of the Flash File Format Specification respectively so the version number is
 * required to encode the nodes correctly.
 * </p>
 *
 * <p>
 * <b>Frames, Buttons or Movie Clips</b><br/>
 ...
 * for writing a script, either specify the actions for an individual button
 * event or for all the events that a button responds to in a single script.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * For specifying multiple events ActionScript provides the on() block
 * construct. When used in a script the nodes are encoded as an array of button
 * event objects and are added to an FSDefineButton2 object:
 * </p>
 *
 * <pre>
 ...
 *
 * <p>
 * NOTE: Although a script can contain more than one on() block, other
 * statements cannot be mixed with on() statements. The parser will generate an
 * error.
 * </p>
 *
 * <p>
 * For movie clip events the situation is similar to generating actions for
 ...
 * trees of ASNodes can be constructed 'manually' - again eliminating the
 ...
 * parse scripts and display a list of ASNode objects that a script is
 * translated into.
 * </p>
 *
 * <p>
 * <b>Search Paths</b>\n ActionScript supports #include directives which allow
 * scripts to be nested. When the script is preprocessed the current working
 * directory is checked for for the file specified. The parser also supports
 * search paths which is a list of directories which should also be checked for
 * the file.
 * </p>
 *
 * <p>
 * Paths may be specified as an array of path names or as a single string with
 * paths separated by the system-dependent path separator character - ';' on
 * Windows platforms and ':' for Unix compatible platforms.
 * </p>
 *
 * <p>
 * <b>Error Handling</b><br/>
 * The parser will throw a ParseException if it detects an error in the syntax
 * of the script it is parsing. The ParseException contains information on the
 * token that triggered the error however this can be cryptic. More useful
 * information is maintained by the parser which has four methods for
 * identifying the line in the script that triggered the error:
 * </p>
 *
 * <p>
 * getFilename() returns the name of the file that contained the script that
 * caused the error. Scripts are passed as strings to the parser however
 * #include directives can be used to load files containing ActionScript and can
 * be nested to any depth. If an error occurs in the script passed directly to
 * the parser then this method will return an empty string "", otherwise the
 * name of the file that contains the line that caused the error is returned.
 * </p>
 *
 * <p>
 * getError() returns a string error code that identifies the type of error that
 * occured, e.g. "FileNotFound". The string can be used as a key to generate
 * localized error message. The ASParser class contains a list of error keys.
 * </p>
 *
 * <p>
