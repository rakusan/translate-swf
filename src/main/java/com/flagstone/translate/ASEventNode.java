...
public class ASEventNode extends ASNode {
    ...
    public void generateEvent(ASContext info, List<Action> list) {
        ...
        for (int i = 0; i < count; i++)
            get(i).generate(info, array);

        array.add(BasicAction.END);
        list.add(new EventHandler(events, key, array));
    }
}
